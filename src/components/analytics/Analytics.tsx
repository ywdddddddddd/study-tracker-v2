...
ChartJS.defaults.animation = { duration: 800, easing: 'easeOutQuart' } as any;
...
export default function AnalyticsPage() {
  ...
  const chartOpts = () => ({
    ...
    plugins: { legend: { position: 'bottom' as const, labels: { font: { size: 10 }, boxWidth: 12, padding: 8 } }, title: { display: false } },
        ...
        <Card>
          ...
          <CardContent>
            {pieValues.reduce((a, b) => a + b, 0) > 0 ? (
              <div className="h-48">
                <Doughnut data={{ labels: pieLabels, datasets: [{ data: pieValues, backgroundColor: TASK_COLORS, borderWidth: 0 }] }} options={{ ...chartOpts(), cutout: '55%', plugins: { legend: { position: 'right' as const, labels: { font: { size: 9 }, boxWidth: 10, padding: 4 } } } }} />
              </div>
